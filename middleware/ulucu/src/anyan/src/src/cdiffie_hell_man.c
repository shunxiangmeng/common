...
static int GetDataBlock_index(int socket_fd)
{
    int i = 0;
    do 
    {
...
int InitDiffieHellman(UINT nSize, int socket_fd)
{
	...
	if(index >= MAX_SOCET_FD)return -1;
    }while(1);
...
int MakePrime(DWORD socket_fd)
{
    ...
    return BNMakePrime(dif_hellman_list._data[index].p_, 
	    dif_hellman_list._data[index].size_,
	    NULL,
	    0,
	    &dif_hellman_list._data[index].dh_crypt_data);
}
...
int Get_exchangekey_(int socket_fd, char *pS_)
{
	...
	//DEBUG(" S1:%x",&(dif_hellman_list._data[index].S1_));
...
int Set_Connect_status(DWORD socket_fd,int status)
{
	...
	if(index == -1) break;
	dif_hellman_list._data[index].connecting = status;
	return 0;
    } while (0);
