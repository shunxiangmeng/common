    ...
    while (!sdk_get_handle(0)->exit_flag)
    {
        ...
        case EN_MODE_REVCE_DATA:
            ...
            if (iRet < 0) {
                ...
                if (err != 10060) {
                    dev_discovery_mode = EN_MODE_SOCK_ERROR;
                    ERRORF("device_discovery_thread()-->recv errno:%d\n", err);
                }
                #else
...
uint32 get_device_flag(st_ay_dev_info *pdevinfo)
{
    ...
    if(timenow->tm_hour == 12 || timenow->tm_hour == 0 )
    {
		if ( timenow->tm_min <= 2)
		{
			if (pdevinfo->sync_time_flag == 0 )
			{
				flag |= 0x01;
			}
		}
...
static int get_entry_loginInfo(st_ay_dev_info *pdevinfo)
{
    ...
    int rslt = 256;
    int len = ZBase64Decode(entry_token_buf.data.token, strlen(entry_token_buf.data.token),
    ...
    TRACEF("GetToken_OK|get entry token ok!\n");
    return 0;
}
...
static int process_regist_msg(int sockfd, st_ay_entry_ctrl *pentries)
{
    ...
    {// send state
        ...
        if (pentries->addr[i].wtout == 0) {
            TRACEF("%s|UDP_OK|send tout = %d\n", ipstr, pentries->addr[i].wtout);
        } else {
            DEBUGF("%s|UDP_OK|send tout = %d\n", ipstr, pentries->addr[i].wtout);
        }
...
static int get_ipconfig(st_ay_entry_ctrl *list)
{
    ...
    int nRet = ayentry_get_entryserver(&serverlist);
    if (nRet != -1) {
        ...
        TRACEF("GetTrackerConfig_OK|serverlist num = %d\n", serverlist.num);
        return  0;
    }
    ERRORF("GetTrackerConfig_FAIL|get entry servers fail!\n");
    return -1;
}
...
static int get_device_id(st_ay_dev_info *pdevinfo)
{
    ...
    if (ret == 0) {
        ...
        TRACEF("Register_OK|register device id: %s\n", szdevice_id);
    ...
    ERRORF("Register_FAIL|register device id fail.\n");
    return -1;
}
