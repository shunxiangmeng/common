...
int ayentry_get_token(entry_token* ou_token, entry_request_param *param)
{
    ...
    ret = get_entryserver_logininfo_byweb(url,ou_token,param);
    if(ret < 0)
    {
...
static int get_deviceid_byjson(const char* in_content,int in_content_len,char* pou_deviceid)
{
    ...
    do 
    {
		...
		if(!pchild)
		{
			ERRORF("{{DeviceCommEvent}}Register_FAIL|get code fail!\n");
			break;
		}
		sdk_get_handle(0)->devinfo.resp_json_code = pchild->valueint;
		...
		if(!pchild)
		{
			ERRORF("{{DeviceCommEvent}}Register_FAIL|get message fail!\n");
			break;
		}
		...
		if(!pdata)
		{
			ERRORF("{{DeviceCommEvent}}Register_FAIL|get data fail!\n");
			break;
		}
		...
		if (!pchild || !pchild->valuestring)
		{
			ERRORF("{{DeviceCommEvent}}Register_FAIL|get did fail!\n");
			break;
		}
		if(pchild->valuestring) strcpy(pou_deviceid,pchild->valuestring);

		pchild = ayJSON_GetObjectItem(pdata,"dev_flag");
		if (!pchild || !pchild->valuestring)
		{
			ERRORF("{{DeviceCommEvent}}Register_FAIL|get devflag fail!\n");
			break;
		}
...
static int device_register_byweb(const char* pin_url,char* pou_deviceid,Dev_SN_Info *param)
{
    ...
    {
		ERRORF("{{DeviceCommEvent}}Register_FAIL|DES_Encrypt failed\n");
		return -1;
    }
    if(hex_encode((unsigned char*)des_reginfo, des_len, cipher_reginfo,sizeof(cipher_reginfo)) < 0)
    {
		ERRORF("{{DeviceCommEvent}}Register_FAIL|hex_encode failed, %d\n", des_len);
		return -1;
    }
    ...
    if (pJonsContent == NULL)
    {
		ERRORF("{{DeviceCommEvent}}Register_FAIL|request_webserver_content:%s fail\n",entry_token_url);
		return -1;
    }
    ...
    {
		free(pJonsContent);
		ERRORF("{{DeviceCommEvent}}Register_FAIL|get_deviceid_byjson fail!\n");
		return -1;
    }
    free(pJonsContent);
    return 0;
