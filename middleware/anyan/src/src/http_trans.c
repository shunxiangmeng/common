...
int http_trans_connect(http_trans_conn *a_conn)
{
  ...
  if(a_conn->hostinfo) ayutil_add_host(a_conn->host,a_conn->saddr.sin_addr.s_addr);
  return 0;
ec:
  return -1;
}
...
http_trans_conn_new(void)
{
  ...
  if(l_return!=NULL)
  {
      ...
      else printf("malloc l_return->io_buf_chunksize fail:%s\n",strerror(errno));
      /* make sure the socket looks like it's closed */
      l_return->sock = -1;
  }
  else printf("malloc http_trans_conn fail!\n");
  return l_return;
}
