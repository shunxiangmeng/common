...
static constexpr std::array<uint32_t, 16> kShifts = {{7, 12, 17, 22, 5, 9, 14, 20, 4, 11, 16, 23, 6, 10, 15, 21}};
// The initial intermediate data.
static constexpr IntermediateData kInitialIntermediateData{0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476};
//////////////////////////////////////////////////////////////////////////////
...
struct MD5CE {
    ...
    static uint8_t GetPaddedMessageByte(const char *data, const uint32_t n, const uint32_t m, const uint32_t i) {
        if (i < n) {
    ...
    static uint32_t GetPaddedMessageWord(const char *data, const uint32_t n, const uint32_t m, const uint32_t i) {
    ...
    static RoundData GetRoundData(const char *data, const uint32_t n, const uint32_t m, const uint32_t i) {
    ...
    static uint32_t CalcF(const uint32_t i, const uint32_t b, const uint32_t c, const uint32_t d) {
        if (i < 16) {
            return d ^ (b & (c ^ d));
        } else if (i < 32) {
            return c ^ (d & (b ^ c));
    ...
    static uint32_t CalcF(const uint32_t i, const IntermediateData &intermediate) {
    ...
    static IntermediateData ApplyStep(const uint32_t i, const RoundData &data, const IntermediateData &intermediate) {
    ...
    static IntermediateData Add(const IntermediateData &intermediate1, const IntermediateData &intermediate2) {
    ...
    static IntermediateData ProcessMessage(const char *message, const uint32_t n) {
    ...
    static uint64_t Hash64(const char *data, uint32_t n) {
        ...
        return (static_cast<uint64_t>(SwapEndian(intermediate.a)) << 32) | static_cast<uint64_t>(SwapEndian(intermediate.b));
    }
